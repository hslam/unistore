...
func stateSize(key, val []byte) int {
	return entryHdrSize + 8 + 2 + 8 + len(key) + len(val)
}

func (w *walWriter) appendState(regionID uint64, key, val []byte, version uint64) {
	...
	w.buf = sstable.AppendU64(w.buf, version)
