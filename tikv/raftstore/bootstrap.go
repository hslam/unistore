...
func writePrepareBootstrap(engines *Engines, region *metapb.Region) error {
	...
	log.S().Infof("shard %d:%d save shard meta from writePrepareBootstrap. meta bin %d, seq %d",
		ingestTree.ChangeSet.ShardID, ingestTree.ChangeSet.ShardVer, len(csBin), ingestTree.ChangeSet.Sequence)
